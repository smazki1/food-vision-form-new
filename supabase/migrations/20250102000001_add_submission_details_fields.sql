-- Add missing submission detail fields to customer_submissions table
-- These fields are accepted by the RPC function but not currently stored

BEGIN;

-- Add missing fields for storing submission details
ALTER TABLE public.customer_submissions 
ADD COLUMN IF NOT EXISTS description TEXT,
ADD COLUMN IF NOT EXISTS category TEXT,
ADD COLUMN IF NOT EXISTS ingredients TEXT[];

-- Update the public_submit_item_by_restaurant_name RPC function to store these fields
DROP FUNCTION IF EXISTS public.public_submit_item_by_restaurant_name(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], TEXT[], TEXT[], TEXT[], TEXT, TEXT, TEXT);

-- Create the updated function with proper field storage
...
BEGIN
  ...
  RAISE LOG 'Additional details - description: %, category: %, ingredients: %', p_description, p_category, p_ingredients;
  ...
  -- Create the submission in customer_submissions table with all details
  ...
  RAISE LOG 'Created submission: % with details: description=%, category=%, ingredients=%', 
    v_submission_id, p_description, p_category, p_ingredients;
...
COMMENT ON COLUMN public.customer_submissions.description IS 'Description/ingredients provided with the submission';
COMMENT ON COLUMN public.customer_submissions.category IS 'Category for dishes and drinks';
COMMENT ON COLUMN public.customer_submissions.ingredients IS 'Ingredients array for cocktails';
