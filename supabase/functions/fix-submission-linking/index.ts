...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Drop the old function and create the corrected one
    const fixSQL = `
      -- Drop the old function first
      DROP FUNCTION IF EXISTS public.public_submit_item_by_restaurant_name(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB);

      -- Create the corrected function that works with customer_submissions
      ...
      BEGIN
        ...
        v_result := json_build_object(
          ...
          'message', CASE 
            WHEN v_client_found THEN 'Submission linked to existing client'
            WHEN v_new_lead_created THEN 'New lead created and submission linked'
            ELSE 'Submission linked to existing lead'
          END
        );
      ...
      GRANT EXECUTE ON FUNCTION public.public_submit_item_by_restaurant_name(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], TEXT[], TEXT, TEXT, TEXT) TO anon;
      GRANT EXECUTE ON FUNCTION public.public_submit_item_by_restaurant_name(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[], TEXT[], TEXT, TEXT, TEXT) TO authenticated;
    `;
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'RPC function fixed successfully' 
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
    ...
    console.error('Error in fix-submission-linking function:', error);
    return new Response(JSON.stringify({ error: error.message }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 500,
    });
