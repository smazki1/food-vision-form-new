...
/**
 * Helper function to automatically deduct servings when submission reaches completion stages (customer version)
 */
async function handleCustomerAutomaticServingDeduction(submissionId: string, submissionData: any) {
  try {
  ...
  return useMutation({
    mutationFn: async ({ submissionId, status }: { submissionId: string; status: SubmissionStatusKey }) => {
      ...
      if (isServingDeductionStatus(status)) {
        await handleCustomerAutomaticServingDeduction(submissionId, updatedSubmission);
      }
